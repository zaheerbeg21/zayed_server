...
def validate_url(request):
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            # os.rmdir('xml_files')
            if os.path.exists('xml_files') and len('xml_files') > 0:
                for i in os.listdir('xml_files'):
                    if i.endswith('.xml'):
                        print("xml> ", i)
                        os.remove('xml_files' + os.sep + i)
            if os.path.exists('json_files') and len(
                    'json_files') > 0:
                for i in os.listdir('json_files'):
                    if i.endswith('.json'):
                        print("json> ", i)
                        os.remove('json_files' + os.sep + i)
            # shutil.rmtree('xml_files')
            else:
                os.mkdir('xml_files')
                os.mkdir('json_files')
            for pdf in os.listdir(pdf_path):
                # print("in pdf dir ", pdf_path + os.sep + pdf)
                list_of_links = pdf_link_extract(pdf_path + os.sep + pdf)
                for ln in list_of_links:
                    print("ln-", ln[0])
                    ext_ = ln[0].split('.')[-1]
                    if ext_ and ln and ext_ == 'xml':
                        print("link-", ln[0])
                        url, tag_lst = xml_val(ln[0], 'xml_files')
                        file_name = qdict.getlist('xml_files')
                        print("xml >>>", file_name)
                        ...
                        return render(request, 'home/upload.html', context)
                    elif ext_ and ln and ext_ == 'json':
                        print("in json of fileupload- ", ln[0])
                        result, jsonData, keys = json_val(ln[0])
                        context = {
                            'tag_lst': keys,
                            'color': 'green',
                            'msg': ln[0] + '- is valid!',
        ...
        if url:
            ...
            if ext_ and url and ext_ == 'xml':
                # url_nm = url.rsplit('/', 1)[1]
                # data = requests.get(url)
                # xml_file = 'xml_files' + os.sep + url_nm
                # with open(xml_file, 'wb') as xml_:
                #     xml_.write(data.content)
                #
                # tag_lst = fetch_tag(xml_file)
                # try:
                #     parsefile(url)
                #     print("XML is well-formed", url)
                #
                ...
                os.remove('xml_files')
                os.mkdir('xml_files')
                url, tag_lst = xml_val(url, 'xml_files')
                context = {
                    'color': "green",
                    'msg': url + '- is valid!',
                    'tag_lst': tag_lst,
                ...
                result, jsonData, keys = json_val(url)
                ## to write json from url
                # json_file = 'json_files' + os.sep + url_nm
                # with open(json_file, 'w', encoding='utf-8') as json_:
                #     json.dump(data, json_)
                context = {
